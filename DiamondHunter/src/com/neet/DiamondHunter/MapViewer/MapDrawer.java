...
/**
 * MapDrawer is responsible for everything related to manipulating and drawing
 * on the canvas.
 */
public class MapDrawer {
	...
	// diamond coordinates
	private int diamonds[][] = { { 20, 20 }, { 12, 36 }, { 28, 4 }, { 4, 34 }, { 28, 19 }, { 35, 26 }, { 38, 36 },
	...
	public void drawItems(int boatX, int boatY, int axeX, int axeY) {
		// Draw the axe
		graphicsContext.drawImage(itemSprite, 0, 16, tileSize, tileSize, ((boatX - colStart) * winTileSize),
				((boatY - rowStart) * winTileSize), winTileSize, winTileSize);

		// Draw the boat
		graphicsContext.drawImage(itemSprite, 16, 16, tileSize, tileSize, ((axeX - colStart) * winTileSize),
				((axeY - rowStart) * winTileSize), winTileSize, winTileSize);

		graphicsContext.setStroke(Color.BLACK);
		if (isBoatHighlighted) {
			graphicsContext.strokeRect((boatX - colStart) * winTileSize, (boatY - rowStart) * winTileSize, winTileSize,
					winTileSize);

		}
		if (isAxeHighlighted) {
			graphicsContext.strokeRect((axeX - colStart) * winTileSize, (axeY - rowStart) * winTileSize, winTileSize,
					winTileSize);
		}
	}
	...
	public void drawAvatar() {
		// Draw the avatar
		graphicsContext.drawImage(avatarSprite, 0, 0, tileSize, tileSize, ((17 - colStart) * winTileSize),
				((17 - rowStart) * winTileSize), winTileSize, winTileSize);
	}
	...
	public void drawDiamonds() {
		...
		for (int i = 0; i < 15; i++) {
			graphicsContext.drawImage(diamondSprite, 0, 0, tileSize, tileSize,
					((diamonds[i][1] - colStart) * winTileSize), ((diamonds[i][0] - rowStart) * winTileSize),
					winTileSize, winTileSize);
		}
	}
	...
	/**
	 * Setter for isAxeHighlighted Set Axe to be highlighted
	 *
	...
	/**
	 * Setter for isBoatHighlighted Set boat to be highlighted
	 *
	 * @param highlight true to highlight, false to de-highlight
	 */
	...
	/**
	 * Draw a box around current cursor location on the map to highlight your
	 * current cursor location on the map, and also show each tile's boundaries for
	 * better usability.
	 * 
	...
	public void drawCursorHighlight(int xCo, int yCo) {
		...
		graphicsContext.strokeRect((xCo - colStart) * winTileSize, (yCo - rowStart)* winTileSize, winTileSize, winTileSize);
	}
}